...
public final class KnitExtractor {
    ...
    /// entry whose `name` is not in the set is skipped (its zstd
    /// blocks aren't decoded, its output file isn't created, parent
    /// directories not covered by another selected entry aren't
    /// materialised). The filter is matched against the archive's
    /// stored entry names verbatim; use
    /// `KnitReader(url:).archive.entries.map(\.name)` (or
    /// `knit unpack --list`) to discover what to pass.
    ///
    /// Mirrors `ZipExtractor.entryFilter` — see that doc-block for
    /// the full rationale + "empty set throws" contract. The same
    /// failure-mode-protection logic applies here: a typo'd
    /// `--entry FOO.txt` against an archive containing `foo.txt`
    /// fails loudly with the list of names that didn't match.
    ...
    public func extract(archive: URL, to destDir: URL) throws -> Stats {
        ...
        let allEntries = reader.archive.entries
        let entries: [KnitEntry]
        if let filter = entryFilter {
            if filter.isEmpty {
                throw KnitError.formatError(
                    ".knit: entryFilter is empty — pass nil for full extract")
            }
            let availableNames = Set(allEntries.map(\.name))
            let missing = filter.subtracting(availableNames)
            if !missing.isEmpty {
                throw KnitError.formatError(
                    ".knit: entries not found in archive: " +
