...
**Knit.app passes `--no-post-verify` by default** when extracting via Finder Quick Actions (the rationale matches the analysis above — APFS + NVMe make the re-read pass essentially defence-in-depth with no realistic failure-mode coverage). The CLI's default remains the safer path (post-verify on); pass `--no-post-verify` yourself if you want the GUI's faster behaviour from the terminal too.
...
- **Pack on M5 Max is now CPU-bound, not write-bound.** Pre-PR-#77, an 80 GB Windows VM image pack spent 70 % of main-thread wall in single-block `write(2)` syscalls — the NVMe controller capped at effectively queue-depth 1. PR #77 replaced the per-block writes with 32 MiB `writev(2)` gather syscalls; the resulting `sample` trace pins the bottleneck on the worker pool's CPU work instead (~80 % `libdeflate_crc32`, ~20 % `ZSTD_compress` on incompressible data). The hardware ceiling is now the 18 P-core's CRC compute bandwidth, not the SSD. Free CPU compute on this tier **does** translate to faster pack now; moving CRC to the GPU at per-entry scale (large VM entries sit above the 1 GiB dispatch-amortisation point) is the next investigation candidate.
